...
createRound :: DM.Map Int [Team] -> [Game]
createRound teamMap = DM.foldrWithKey (\k a b -> b ++ [(Game (a !! 0) (a !! 1) k)]) [] teamMap
...
createBracket teams = map (\(seeds, region, gmId) -> createGame seeds region gmId teamMap) (zip3 seedMatchups regions gameNums)
...
seedMatchups = concat $ replicate 4 [(1, 16), (8, 9), (5, 12), (4, 13), (6, 11), (3, 14), (7, 10), (2, 15)]
gameNums = [1..32]
regions = concat [replicate 8 "South", replicate 8 "East", replicate 8 "West", replicate 8 "Midwest"]
